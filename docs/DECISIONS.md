...
| `tide list` | Print git-tracked dotfiles (`git ls-files` of the tide repo) mapped to home source paths via the watch list, one per line. **Source of truth is the repo**, not the watch list — a watched-but-unsynced (or secret-blocked) file won't appear; a file left in the repo after `rm` will. |
